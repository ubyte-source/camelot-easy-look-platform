...
(function (window) {
    ...
    for (let item = 0; item < window.page.tables.read.fields.length; item++) switch (window.page.tables.read.fields[item].name) {
        case 'label':
            window.page.tables.read.fields[item][Infinite.Body.TD.handling()] = function (array) {
                let mapped = {},
                ...
                let result = document.createElement('div'), node = document.createTextNode(label);
                result.className = 'result';
                result.appendChild(node);
                if (label === Infinite.Plugin.Text.void())
            ...
            break;
        case 'icon':
            window.page.tables.read.fields[item][Infinite.Body.TD.handling()] = function (value) {
                ...
                if (icon_text === Infinite.Plugin.Text.void()) {
                    let icon_node = document.createTextNode(icon_text),
    ...
    widgets.read.setRequestUrl('/api/document/output/read');
    widgets.read.setResponseKey('data');
    widgets.read.setResponseUnique('id_project');
    ...
    if (window.page.checkPolicy(window.page.application + '/document/output/action/insert')) {
        let add = new Button();
        add.addStyle('flat');
        add.getIcon().set('add');
    ...
    instructions.className = 'bolder';
    instructions.appendChild(document.createTextNode(window.page.getTranslate('modal.delete.instructions')));
    ...
    widgets.modal.buttons.submit.onClick(function () {
        ...
        for (let x in window.choosed) {
            let xhr = new WXmlHttpRequest(),
                url = '/api/document/output/delete'
                    + String.fromCharCode(47)
    ...
    if (window.page.checkPolicy(window.page.application + '/document/output/action/delete')) {
